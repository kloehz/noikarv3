...
## Code-grounded potential CPU hotspots

These are measurement targets, not optimization recommendations:

- `core/AIComponent.gd`: target acquisition scans hostile groups; patrol/chase/attack decisions; ally avoidance over nearby mob/pet groups.
- `core/CombatComponent.gd`: combat processing is already instrumented through `PerfProbe` NPC cost timers when enabled.
- `core/LogicComponent.gd`: movement preparation, slide, and flush child timers are reported as instrumentation spans under the same NPC cost opt-in.
- `common/perf_probe.gd`: Netfox custom monitors and rollback signal aggregation are reported every five seconds.
- `common/match_manager.gd`: fixed population spawning and observable rollback/snapshot entity counting for benchmark startup validation.
